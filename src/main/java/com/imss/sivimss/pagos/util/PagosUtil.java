...
public class PagosUtil {
	...
	private static final String CONSULTA_TABLA = "SELECT T.*\r\n"
			...
			+ " PB.IND_GEN_PAGARE AS generarPagare,\r\n"
			+ " PB.ID_PLATAFORMA AS idPlataforma\r\n"
			+ " FROM SVT_PAGO_BITACORA PB\r\n"
			+ " INNER JOIN SVC_FLUJO_PAGOS FP ON FP.ID_FLUJO_PAGOS = PB.ID_FLUJO_PAGOS\r\n"
			+ " INNER JOIN SVC_ORDEN_SERVICIO OS ON OS.ID_ORDEN_SERVICIO = PB.ID_REGISTRO\r\n"
			...
			+ " INNER JOIN SVC_ESTATUS_PAGO EOSP ON EOSP.ID_ESTATUS_PAGO = PB.CVE_ESTATUS_PAGO\r\n"
			+ " WHERE PF.ID_ESTATUS_CONVENIO IN (1,2) AND PB.ID_FLUJO_PAGOS = '2'\r\n"
			+ " )\r\n"
			+ "UNION ALL\r\n"
			+ " (SELECT \r\n"
			+ " PB.ID_PAGO_BITACORA AS idPagoBitacora, \r\n"
			...
			+ " WHERE RPF.ID_ESTATUS IN (1,2) AND PB.ID_FLUJO_PAGOS = '3'\r\n"
			+ " )\r\n"
			+ ") T";
	...
	public String registroDetalle(String idPagoBitacora, String formatoFecha){
		...
		StringBuilder query = new StringBuilder("SELECT \r\n"
				...
				+ "INNER JOIN SVC_PERSONA PER ON PER.ID_PERSONA = CON.ID_PERSONA\r\n"
				+ "WHERE\r\n"
				+ "OS.ID_ORDEN_SERVICIO = idRegistro\r\n"
				+ "AND PB.ID_FLUJO_PAGOS = 1\r\n"
				+ ") AS nss,\r\n"
				+"(\r\n"
				+ "SELECT\r\n"
				...
				+ "FROM\r\n"
				+ "SVC_FINADO FIN\r\n"
				+ "INNER JOIN SVC_ORDEN_SERVICIO OS ON OS.ID_ORDEN_SERVICIO = FIN.ID_ORDEN_SERVICIO\r\n"
				+ "INNER JOIN SVT_PAGO_BITACORA PB ON PB.ID_REGISTRO = OS.ID_ORDEN_SERVICIO\r\n"
				+ "WHERE\r\n"
				+ "OS.ID_ORDEN_SERVICIO = idRegistro\r\n"
				+ "AND PB.ID_FLUJO_PAGOS = 1\r\n"
